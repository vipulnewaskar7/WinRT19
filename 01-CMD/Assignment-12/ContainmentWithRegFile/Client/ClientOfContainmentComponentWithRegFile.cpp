...
ISummation *pISummation = NULL;
ISubtraction *pISubtraction = NULL;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg,WPARAM wParam, LPARAM lParam)
{
    ...
    switch(iMsg)
    {
        case WM_CREATE:
            ...
            hr = CoCreateInstance(CLSID_Calculator, NULL, CLSCTX_INPROC_SERVER, IID_ISummation, (void**) &pISummation);
            if (FAILED(hr))
            {
                MessageBox(hwnd, TEXT("ISummation Interface cannot be found, program will exit now."), TEXT("Error"), MB_OK);
                DestroyWindow(hwnd);
            }
            ...
            pISummation->SumOfTwoIntegers(iNum1, iNum2, &iNum3);
            wsprintf(str, TEXT("Sum of %d and %d is %d"),iNum1, iNum2, iNum3);
            MessageBox(hwnd, str, TEXT("Answer of Summation"),MB_OK);
            
            pISummation->QueryInterface(IID_ISubtraction, (void**) &pISubtraction);
            if(FAILED(hr))
            {
